...
class TopKSparseMoELayer(nn.Module):
    def __init__(self, num_experts, input_dim, hidden_dim, output_dim, top_k=2):
        super(TopKSparseMoELayer, self).__init__()
        self.top_k = top_k
        self.experts = nn.ModuleList([SparseExpert(input_dim, hidden_dim, output_dim) for _ in range(num_experts)])
    ...
    def forward(self, x):
        ...
        expert_outputs = [self.experts[idx](x_reshaped) for idx in range(len(self.experts))]
        expert_outputs = torch.stack(expert_outputs, dim=1)  # Shape: [batch_size * seq_len, num_experts, output_dim]

        result = torch.zeros_like(expert_outputs[:, 0, :])  # 初始化输出结果
        for i in range(self.top_k):
            weights = F.softmax(top_k_values[:, i], dim=0).unsqueeze(-1)  # 正确的softmax权重
            result += weights * expert_outputs[torch.arange(expert_outputs.size(0)), top_k_indices[:, i], :]
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, patch_size=None, num_experts=2, expert_input_dim=768,
                 expert_hidden_dim=1024, expert_output_dim=768, top_k=2):
        super().__init__(config, embed_tokens)

        self.patch_num, self.patch_dim = patch_size
        self.image_dense = nn.Linear(self.patch_dim, config.d_model)
        self.mha_layer = torch.nn.MultiheadAttention(embed_dim=config.hidden_size, num_heads=1, batch_first=True)
    ...
    def forward(self, input_ids=None, attention_mask=None, encoder_hidden_states=None, encoder_attention_mask=None,
                inputs_embeds=None, image_ids=None, head_mask=None, cross_attn_head_mask=None, past_key_values=None,
