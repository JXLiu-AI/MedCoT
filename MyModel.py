...
class TopKSparseMoELayer(nn.Module):
    def __init__(self, num_experts, input_dim, output_dim, top_k=2):
        ...
        self.top_k = top_k
        self.experts = nn.ModuleList([SparseExpert(input_dim, output_dim) for _ in range(num_experts)])
    ...
    def forward(self, x):
        ...
        # print(top_k_values[: ,0].mean())
        # print(gate_scores[:,0].mean())
        # print(gate_scores[:,1].mean())
        # print(gate_scores[:,2].mean())
        # print(gate_scores[:,3].mean())
        # print(gate_scores[:,4].mean())
        # print(gate_scores[:,5].mean())
        # print(gate_scores[:,6].mean())
        # print(gate_scores[:,7].mean())
        # print(gate_scores[:,8].mean())
        # print(gate_scores[:,9].mean())
        # import pdb;pdb.set_trace()
        # print(top_k_values[: ,1].mean())
        # import pdb;pdb.set_trace()
        expert_outputs = [self.experts[idx](x_reshaped) for idx in range(len(self.experts))]
        expert_outputs = torch.stack(expert_outputs, dim=1)

        result = torch.zeros_like(expert_outputs[:, 0, :])
        for i in range(self.top_k):
            weights = F.softmax(top_k_values[:, i], dim=0).unsqueeze(-1)
...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, patch_size=None):
        ...
        self.gate_dense = TopKSparseMoELayer(6, 2 * config.hidden_size, config.hidden_size, top_k=2)
