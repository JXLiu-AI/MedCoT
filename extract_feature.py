...
def get_model(_img_type):
    ...
    if _img_type == "vit":
        _model = timm.create_model("vit_large_patch32_384", pretrained=True, num_classes=0)
        # Resolve the data configuration and create the appropriate transform for the vit_model
        ...
        # cooelf_path = "/path/to/.cache/torch/hub/cooelf_detr_main"
        cooelf_path = "/path/to/.cache/torch/hub/cooelf_detr_main"
...
def extract_features(_model, _transform, _input_image, _img_type, _device):
        ...
        with torch.no_grad():
            # Extract features using the vit_model
            if _img_type == 'vit':
                return _model.forward_features(input)
            elif _img_type == 'detr':
...
if __name__ == "__main__":
    ...
    #     for idx, image_path in enumerate(images_path):
    #         if image_path.endswith('.jpg'):
    #             # image = images_dir + image_path + "/source.jpg"
    ...
    img_type = "detr"
    images_dir = "/data/jiaxiang/8st/mm_cot/PathVQA/PathVQA-dataset/train"
    images_path = os.listdir(images_dir)
    output_file_path = "/data/jiaxiang/8st/mm_cot/PathVQA/PathVQA-dataset"
    print(f"There are {len(images_path)} images in {images_dir}.")
    ...
    print(vision_features.shape)
    torch.save(vision_features, os.path.join(output_file_path, f"{img_type}.pth"))
    with open(os.path.join(os.path.dirname(output_file_path), "name_map.json"), 'w') as outfile:
        json.dump(name_map, outfile, indent=4, ensure_ascii=False)
    print("Files have been written...")
